...
4. **Content Library** (`/content-library`) — Browse all content assets by funnel stage, search by content ID, infinite scroll, URL preview. Includes a **Content Comparison** tool (collapsible) — a B2B content classifier for Sage. Upload two PDFs: each is classified via Claude AI (content type, funnel stage, product, industry, topic) with rule-based fallback if AI fails (flagged as fallback). Weighted matching finds top 5 similar assets from the library (product overlap 40%, topic keyword overlap 30%, industry overlap 15%, content similarity 15%), filtered to top-quartile performers by stage-appropriate metric (pageviews for TOFU, leads for MOFU, SQOs for BOFU). Aggregate benchmarks (min/max/mean/median) computed across the comparison set for all performance metrics plus avg CTA count. Shows vocabulary overlap, side-by-side classification comparison, and extracted text preview. **Core analysis** runs AI strategist (senior content strategist system prompt, 500 max tokens) returning: campaign readiness score (0-100) with breakdown (structure, CTAs, topic depth, format), performance forecast with projected range and confidence, 3-5 prioritized recommendations each citing a Content ID, reusability assessment (overlap, cannibalization risk, repurposing opportunities), and a top-action sentence. Fallback scoring from benchmarks alone if AI fails. Results cached 30 min by filename+size+content hash. Full pipeline: extract PDF → classify → match → analyze → return as one response.
