...
-   **Core Pages**:
    ...
    -   **Feedback & My Reports & Admin**: Standard pages for user feedback, customizable reports (with drag-to-resize, add/edit/remove widgets), and administrative tasks like data upload and AI column mapping. **Widget Edit**: Each chart widget has an edit button (pencil icon) in the hover toolbar that opens a modal pre-populated with the widget's current settings (chart type, dimension, measure, stage filter, title, size). Saving preserves the existing grid span unless the user explicitly changes the size. Form fields are shared between Add and Edit modals via `WidgetFormFields` component.
...
-   **Sales Feedback**: Backend system for sales team content feedback. Each entry stores `contentId`, `author`, `tags` (from predefined prospect reaction and content quality lists), optional `note`, and optional `salesforceRef`. Endpoints: `POST /api/sales-feedback`, `GET /api/sales-feedback/:contentId`, `GET /api/sales-feedback/:contentId/stats`. Stats include total count, tag frequency counts, and a sentiment score (positive minus negative tags ratio). Schema in `sales_feedback` table. **Comparison Integration**: Sales feedback is injected into both 2-way and multi-comparison AI prompts and returned as `salesSignal` in comparison API responses (includes `tagCounts`, `topTags`, and up to 10 full `entries` with author/tags/note/salesforceRef/createdAt). The CIA Agent and Content Librarian system prompts include rules to incorporate feedback naturally. The `buildContentLibraryContext()` function includes per-asset feedback summary. Frontend `SalesFeedbackSection` component renders: side-by-side feedback summaries with color-coded tags (green=positive, red=negative), a Sales Signal summary noting stronger SDR endorsement, a tag comparison chart with horizontal bars, and a Rep Notes section showing recent free-form notes with author/date. Empty state displayed when no feedback exists.
...
-   **Schema**: Includes `assets_agg` (content performance), `users`, `conversations`, `messages`, `content_stored`, `comparison_history`, `journey_interactions`, `contact_journeys`, `journey_patterns`, `stage_transitions`, `asset_journey_stats`, and `journey_stage_flows` tables, with custom enums like `funnel_stage`, `comparison_type`, and `comparison_status`.
-   **Journey Data Pipeline**: Two-layer architecture for 400K+ row Eloqua activity data. Layer 1: Raw `journey_interactions` table (written once at upload with SHA-256 hashed emails, dedup, cleaning). Layer 2: Pre-computed summary tables (`contact_journeys`, `journey_patterns`, `stage_transitions`, `asset_journey_stats`, `journey_stage_flows`) built via background `journey-builder.ts` after upload (5 phases: contacts, journeys, patterns/transitions, asset stats, asset-to-asset flows). `asset_journey_stats` includes `funnelStage`, `uniqueContacts`, `entryCount`, `exitCount`, `passThroughCount`. `journey_stage_flows` stores asset-to-asset transition flows with contact counts and avg days between. Stage assignment uses per-interaction funnel stage data (not compressed stage sequences). UI reads from summary tables for instant dashboards. CSV/TSV parsing uses `csv-parse` library for proper quoted field handling. DB indexes on all key query columns. 5-minute in-memory cache on summary endpoints.
-   **Journey Map UI**: 4 tabs — Asset Flow (D3 Sankey diagram with d3-sankey, stage-column layout, interactive hover, min-contacts slider, fetches from `/api/journey/stage-flows`), Content Paths (content transitions table with filters, flow diagram, insight cards), Journey Patterns (sortable by popularity/conversion/speed, expandable details), Top Assets (sortable, stage-filterable, entry/exit/pass-through counts).
...
-   **Claude AI (Anthropic)**: Utilized for:
    ...
    -   Content Librarian for classification, comparison, and recommendations. Has full access to all uploaded content analysis (summaries, topics, CTAs, messaging themes, structure, keyword tags) and recent comparison history via `buildContentLibraryContext()`. Capped at 50 assets with truncated summaries for prompt stability.
