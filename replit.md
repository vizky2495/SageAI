...
- **AI Interaction (Hybrid System)**: Three-tier interaction in `PageChat` component (`client/src/components/page-chat.tsx`):
    - **Trigger**: Bottom-right 48px glassmorphic green circle with "AI" text, pulse animation, ⌘K tooltip on hover.
    - **Spotlight Mode** (⌘K): Centered 640px panel at top 20%, dark overlay 50%. Smart search dropdown with pattern matching (compare, gap, top/best, plan, evaluate). Quick answer mode expands inline (max 50vh). After 3+ messages, shows "Continue in full chat?" prompt.
    - **Full Chat Mode**: Bottom slide-up panel, draggable header, resizable top edge, conversation history sidebar (240px), fullscreen toggle. Dark glassmorphic `rgba(15,15,15,0.97)` bg. Supports file attachments, image lightbox, markdown tables, verdict badges.
