...
- **AI Chat (Command Bar)**: Bottom-centered command bar (`PageChat` component with `variant="commandbar"`) replaces the old sidebar. Fixed pill-shaped bar at bottom (60% width, 56px height, glassmorphic dark bg with green border). Expands into a centered 700px chat window (60vh) on click/focus/⌘K. Conversation history in slide-out panel (History icon), not default view. Starters and suggestions shown for empty chats. Auto-generated conversation titles (5-8 words via AI, first 6 words as fallback). All pages use full width (max-w-[1400px]) with no sidebar eating screen space. Supports file attachments, image lightbox, markdown tables, verdict badges.
- **Pages**:
    - **Hub (`/`)**: Landing page with navigation to core modules.
    - **Content Performance (`/performance`)**: KPI overview and high-level funnel insights. Full-width layout with bottom command bar.
    - **Deep Dive Analytics (`/analytics`)**: Detailed filtering and CTA/channel/product analysis. Full-width layout with bottom command bar.
    - **Content Library (`/content-library`)**: Content browsing, search, and an AI-powered Content Comparison tool. Full-width layout with bottom command bar (Content Librarian agent). The comparison estimates new content performance against existing assets and provides strategic recommendations. PDF upload uses multi-stage extraction (pdfjs primary, raw buffer fallback), processes max 20 pages for large files (up to 50MB), and offers manual content entry as a fallback when extraction fails.
