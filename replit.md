...
- **Authentication** — Login page gates all app access. Two roles: User (validated against `USER_PASSWORD` secret) and Admin (validated against `ADMIN_PASSWORD` secret). Users enter a display name + password; new users are auto-created in the `users` table. Auth context (`client/src/lib/auth.tsx`) provides `{ user, isLoggedIn, isAdmin, login, logout }`. Token stored in `localStorage["cia_token"]`. Admin tokens also added to in-memory `adminTokens` set for `requireAdmin` middleware compatibility. Admin page auto-authenticates for admin users. Logout button on Hub page.
