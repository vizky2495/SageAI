...
4. **Content Library** (`/content-library`) — Browse all content assets by funnel stage, search by content ID, infinite scroll, URL preview. Includes a **Content Comparison** tool (collapsible) — compares existing content (Content A) against new content (Content B) to estimate performance. **Content A**: Searchable asset picker (`/api/assets/search-picker`) that pulls real performance metrics (pageviews, downloads, leads, SQOs, avg time) from the uploaded dataset. If content not found, falls back to PDF upload with manual metric entry fields. **Content B**: Upload a new PDF — classified via Claude AI (content type, funnel stage, product, industry, topic) with rule-based fallback. Stage-based matching finds top 5 similar assets from the library (product overlap 40%, topic keyword overlap 30%, industry overlap 15%, content similarity 15%), filtered to top-quartile performers by stage-appropriate metric (pageviews for TOFU, leads for MOFU, SQOs for BOFU). **Comparison Results**: Side-by-side table showing Content A actual metrics vs Content B estimated metrics with delta percentages. **Core analysis** runs AI strategist (senior content strategist system prompt, 500 max tokens) returning: campaign readiness score (0-100) with breakdown (structure, CTAs, topic depth, format), performance forecast with projected range and confidence based on real dataset medians, 3-5 prioritized recommendations each citing a Content ID with actual metrics, reusability assessment, and a top-action sentence. Fallback scoring from benchmarks alone if AI fails. Results cached 30 min by filename+wordCount+pageCount+content hash. PDF extraction uses `pdfjs-dist` (Mozilla PDF.js) for robust text extraction.
