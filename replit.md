...
- **AI Interaction (Hybrid System)**: Three-tier interaction in `PageChat` component (`client/src/components/page-chat.tsx`):
    - **Trigger**: Bottom-right stacked floating buttons: 48px "AI" circle (opens Spotlight, ⌘K) + 48px chat bubble circle (opens full chat, ⌘J). Chat button always visible; shows minimize icon when chat is open. Both have hover tooltips with shortcut hints.
    - **Spotlight Mode** (⌘K): Centered 640px panel at top 20%, light overlay `rgba(0,0,0,0.3)` with no blur — page content remains legible. CIA agent label/avatar clickable to open full chat. Chat icon button in bar alongside paperclip. Smart search dropdown with pattern matching (compare, gap, top/best, plan, evaluate). Quick answer mode expands inline (max 50vh). After 3+ messages, shows "Continue in full chat?" prompt.
    - **Full Chat Mode** (⌘J): Bottom slide-up panel, no overlay/blur on page content above — users can read and interact with dashboard data while chatting. Panel has `box-shadow: 0 -4px 20px rgba(0,0,0,0.5)` for separation. Draggable header, resizable top edge (40px drag handle, 200px min / 80vh max / 45vh default, height persisted in `localStorage` as `cia-chat-height`), conversation history sidebar (240px), fullscreen toggle. Dark bg `rgba(10,12,10,0.95)`. Supports file attachments, image lightbox, markdown tables, verdict badges.
